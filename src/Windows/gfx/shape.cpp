...
namespace osw32 {
	/*void processContourPaths(window* win, gfx::contour* c, ID2D1PathGeometry* geom) {
		...
		if (c->fill) {
			...
			sink->BeginFigure(D2D1::Point2F(c->initialX, c->initialY), D2D1_FIGURE_BEGIN_HOLLOW);
		}
...
namespace gfx {
	/*void surface::renderShape(shape* s) {
		...
		for (size_t c = 1; c < s->contours.size(); ++c) {
			osw32::processContourPaths(win, &s->contours[c], geom);
		}

		if (s->shapeColor.a != 0.f) {
			osw32::brushCol(&win->ren, s->shapeColor);
			win->ren.target->FillGeometry(geom, win->ren.brush);
		}

		if (s->borderThickness != 0.f && s->borderColor.a != 0.f) {
			osw32::brushCol(&win->ren, s->borderColor);
			win->ren.target->DrawGeometry(geom, win->ren.brush, s->borderThickness);
		}
